 ...
 * <p>
 * This bean uses the 
 * <a href="http://ietf.org/rfc/rfc1658.txt">RFC 1658</a>
 * (CHARACTER-MIB module) and 
 * <a href="http://ietf.org/rfc/rfc1659.txt">RFC 1659</a>
 * (RS-232-MIB module).
 * </p>
 *
 * <p>
 ...
 * @version $Revision: 1.16 $ $Date: 2002/10/29 14:44:38 $
 */
public class AnnexModemStatusBean extends SNMPRunBean 
        implements Observer, TreeNode
{
    private static final String     version_id =
        "@(#)$Id: AnnexModemStatusBean.java,v 1.16 2002/10/29 14:44:38 birgit Exp $ Copyright Westhawk Ltd";
...
public TreeNode getChildAt(int childIndex)
{
    ...
    {
        Enumeration enum = modemHash.elements();
...
public void update(Observable obs, Object ov)
{
    ...
    if (obs instanceof GetNextPdu_vec)
    {
        ...
        if (pduGetNext.isTimedOut() == false)
        {
            if (pduGetNext.getErrorStatus() == AsnObject.SNMP_ERR_NOERROR)
            {
                ...
                if (var[0].getOid().toString().startsWith(charPortIndex))
                {
                    ...
                    if (portStatus == portUP)
                    {
                        ...
                        try
                        {
                            OneGetPdu pduGetOne = new OneGetPdu(context, oid, this);
                        }
                        catch (PduException exc)
                        {
                            System.out.println("PduException " + exc.getMessage());
            ...
            else
            {
                ...
                // we have all the responses from pduGetOne's. Since I
    ...
    else if (obs instanceof OneGetPdu)
    {
        OneGetPdu pduGetOne = (OneGetPdu) obs;
        if (pduGetOne.getErrorStatus() == AsnObject.SNMP_ERR_NOERROR)
        {
